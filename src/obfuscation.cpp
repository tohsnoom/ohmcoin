...
// A helper object for signing messages from Karmanodes
...
// Keep track of the used Karmanodes
std::vector<CTxIn> vecKarmanodesUsed;
// Keep track of the scanning errors I've seen
map<uint256, CObfuscationBroadcastTx> mapObfuscationBroadcastTxes;
// Keep track of the active Karmanode
CActiveKarmanode activeKarmanode;

/* *** BEGIN OBFUSCATION MAGIC - OHMC **********
...
void CObfuscationPool::ProcessMessageObfuscation(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == NetMsgType::DSA) { //Obfuscation Accept Into Pool
        ...
        if (pfrom->nVersion < ActiveProtocol()) {
            ...
            pfrom->PushMessage(NetMsgType::DSSU, sessionID, GetState(), GetEntriesCount(), KARMANODE_REJECTED, errorID);
        ...
        if (!IsCompatibleWithSession(nDenom, txCollateral, errorID)) {
            ...
            pfrom->PushMessage(NetMsgType::DSSU, sessionID, GetState(), GetEntriesCount(), KARMANODE_ACCEPTED, errorID);
            return;
        }
        ...
        CKarmanode* pmn = mnodeman.Find(dsq.vin);
        if (pmn == NULL) return;

        // if the queue is ready, submit if we can
        if (dsq.ready) {
            if (!pSubmittedToKarmanode) return;
            if ((CNetAddr)pSubmittedToKarmanode->addr != (CNetAddr)addr) {
                LogPrintf("dsq - message doesn't match current Karmanode - %s != %s\n", pSubmittedToKarmanode->addr.ToString(), addr.ToString());
                return;
            }
            ...
            if (pmn->nLastDsq != 0 &&
                pmn->nLastDsq + mnodeman.CountEnabled(ActiveProtocol()) / 5 > mnodeman.nDsqCount) {
                LogPrint("obfuscation", "dsq -- Karmanode sending too many dsq messages. %s \n", pmn->addr.ToString());
                return;
            }
            mnodeman.nDsqCount++;
        ...
        if (!pSubmittedToKarmanode) return;
        if ((CNetAddr)pSubmittedToKarmanode->addr != (CNetAddr)pfrom->addr) {
            //LogPrintf("dssu - message doesn't match current Karmanode - %s != %s\n", pSubmittedToKarmanode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (!pSubmittedToKarmanode) return;
        if ((CNetAddr)pSubmittedToKarmanode->addr != (CNetAddr)pfrom->addr) {
            //LogPrintf("dsc - message doesn't match current Karmanode - %s != %s\n", pSubmittedToKarmanode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
...
std::string CObfuscationPool::GetStatus()
{
    ...
    if (chainActive.Tip()->nHeight - cachedLastSuccess < minBlockSpacing || !karmanodeSync.IsBlockchainSynced()) {
    ...
    case POOL_STATUS_ACCEPTING_ENTRIES:
        if (entriesCount == 0) {
            ...
            if (showingObfuScationMessage % 70 <= 40)
                return strprintf(_("Submitted following entries to karmanode: %u / %d"), entriesCount, GetMaxPoolTransactions());
                ...
                suffix = "...";
            return strprintf(_("Submitted to karmanode, waiting for more entries ( %u / %d ) %s"), entriesCount, GetMaxPoolTransactions(), suffix);
        }
    ...
    case POOL_STATUS_QUEUE:
        if (showingObfuScationMessage % 70 <= 30)
            ...
            suffix = "...";
        return strprintf(_("Submitted to karmanode, waiting in queue %s"), suffix);
        ;
    default:
...
//
// Check the Obfuscation progress and send client updates if a Karmanode
//
...
{
    if (!fMasterNode) return; // check and relay final tx only on karmanode
...
//
// How does this work? Messages to Karmanodes come in via "dsi", these require a valid collateral
// transaction for the client to be able to enter the pool. This transaction is kept by the Karmanode
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Ohmcoin and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 OHMC per transaction on average.
            */
            if (r <= 10) {
...
//
// Execute a Obfuscation denomination via a Karmanode.
// This is only ran from clients
//
void CObfuscationPool::SendObfuscationDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, CAmount amount)
{
    if (fMasterNode) {
        LogPrintf("CObfuscationPool::SendObfuscationDenominate() - Obfuscation from a Karmanode is not supported currently.\n");
        return;
    }
    ...
    // we should already be connected to a Karmanode
    if (!sessionFoundKarmanode) {
        LogPrintf("CObfuscationPool::SendObfuscationDenominate() - No Karmanode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return;
    }
...
// Incoming message from Karmanode updating the progress of Obfuscation
...
bool CObfuscationPool::StatusUpdate(int newState, int newEntriesCount, int newAccepted, int& errorID, int newSessionID)
{
    ...
    if (errorID != MSG_NOERR) strAutoDenomResult = _("Karmanode:") + " " + GetMessageByID(errorID);
    ...
    if (newState == POOL_STATUS_ACCEPTING_ENTRIES) {
        if (newAccepted == 1) {
            ...
            sessionFoundKarmanode = true;
            //wait for other users. Karmanode will report when ready
            UpdateState(POOL_STATUS_QUEUE);
        } else if (newAccepted == 0 && sessionID == 0 && !sessionFoundKarmanode) {
            LogPrintf("CObfuscationPool::StatusUpdate - entry not accepted by Karmanode \n");
            UnlockCoins();
            UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);
            DoAutomaticDenominating(); //try another Karmanode
        }
        if (sessionFoundKarmanode) return true;
    }
...
//
// After we receive the finalized transaction from the Karmanode, we must
...
bool CObfuscationPool::SignFinalTransaction(CTransaction& finalTransactionNew, CNode* node)
{
    ...
    // push all of our signatures to the Karmanode
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    if (mnodeman.size() == 0) {
        LogPrint("obfuscation", "CObfuscationPool::DoAutomaticDenominating - No Karmanodes detected\n");
        strAutoDenomResult = _("No Karmanodes detected.");
        return false;
    }
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeOhmcoinAmount * COIN - pwalletMain->GetAnonymizedBalance();
    ...
    // initial phase, find a Karmanode
    if (!sessionFoundKarmanode) {
        ...
        //if we've used 90% of the Karmanode list then drop all the oldest first
        int nThreshold = (int)(mnodeman.CountEnabled(ActiveProtocol()) * 0.9);
        LogPrint("obfuscation", "Checking vecKarmanodesUsed size %d threshold %d\n", (int)vecKarmanodesUsed.size(), nThreshold);
        while ((int)vecKarmanodesUsed.size() > nThreshold) {
            vecKarmanodesUsed.erase(vecKarmanodesUsed.begin());
            LogPrint("obfuscation", "  vecKarmanodesUsed size %d threshold %d\n", (int)vecKarmanodesUsed.size(), nThreshold);
        }
        ...
        if (nUseQueue > 33) {
            ...
            BOOST_FOREACH (CObfuscationQueue& dsq, vecObfuscationQueue) {
                ...
                bool fUsed = false;
                //don't reuse Karmanodes
                BOOST_FOREACH (CTxIn usedVin, vecKarmanodesUsed) {
                    if (dsq.vin == usedVin) {
                        fUsed = true;
                        break;
                    }
                ...
                CKarmanode* pmn = mnodeman.Find(dsq.vin);
                if (pmn == NULL) {
                    LogPrintf("DoAutomaticDenominating --- dsq vin %s is not in karmanode list!", dsq.vin.ToString());
                    continue;
                }

                LogPrintf("DoAutomaticDenominating --- attempt to connect to karmanode from queue %s\n", pmn->addr.ToString());
                lastTimeChanged = GetTimeMillis();

                // connect to Karmanode and submit the queue request
                CNode* pnode = ConnectNode((CAddress)addr, NULL, true);
                if (pnode != NULL) {
                    pSubmittedToKarmanode = pmn;
                    vecKarmanodesUsed.push_back(dsq.vin);
                    ...
                    strAutoDenomResult = _("Error connecting to Karmanode.");
                    dsq.time = 0; //remove node
                    continue;
                }
            }
        ...
        while (i < 10) {
            CKarmanode* pmn = mnodeman.FindRandomNotInVec(vecKarmanodesUsed, ActiveProtocol());
            if (pmn == NULL) {
                LogPrintf("DoAutomaticDenominating --- Can't find random karmanode!\n");
                strAutoDenomResult = _("Can't find random Karmanode.");
                return false;
            }
            ...
            lastTimeChanged = GetTimeMillis();
            LogPrintf("DoAutomaticDenominating --- attempt %d connection to Karmanode %s\n", i, pmn->addr.ToString());
            CNode* pnode = ConnectNode((CAddress)pmn->addr, NULL, true);
            if (pnode != NULL) {
                pSubmittedToKarmanode = pmn;
                vecKarmanodesUsed.push_back(pmn->vin);
                ...
                vecKarmanodesUsed.push_back(pmn->vin); // postpone MN we wasn't able to connect to
                i++;
                continue;
            }
        }

        strAutoDenomResult = _("No compatible Karmanode found.");
        return false;
    }
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100OHMC+1 ( bit on if present )
    // bit 1 - 10OHMC+1
    // bit 2 - 1OHMC+1
    // bit 3 - .1OHMC+1
    // bit 3 - non-denom
...
std::string CObfuscationPool::GetMessageByID(int messageID)
{
    ...
    case ERR_MN_LIST:
        return _("Not in the Karmanode list.");
    case ERR_MODE:
        return _("Incompatible mode.");
    case ERR_NON_STANDARD_PUBKEY:
        return _("Non-standard public key detected.");
    case ERR_NOT_A_MN:
        return _("This is not a Karmanode.");
    case ERR_QUEUE_FULL:
        return _("Karmanode queue is full.");
    case ERR_RECENT:
...
bool CObfuscationQueue::CheckSignature()
{
    ...
    if (pmn != NULL) {
        ...
        if (!obfuScationSigner.VerifyMessage(pmn->pubKeyKarmanode, vchSig, strMessage, errorMessage)) {
            return error("CObfuscationQueue::CheckSignature() - Got bad Karmanode address signature %s \n", vin.ToString().c_str());
        }
...
void CObfuscationPool::RelayIn(const std::vector<CTxDSIn>& vin, const int64_t& nAmount, const CTransaction& txCollateral, const std::vector<CTxDSOut>& vout)
{
    ...
    CNode* pnode = FindNode(pSubmittedToKarmanode->addr);
    if (pnode != NULL) {
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("ohmcoin-obfuscation");
    ...
    while (true) {
        ...
        if (karmanodeSync.IsBlockchainSynced()) {
            ...
            if (c % KARMANODE_PING_SECONDS == 1) activeKarmanode.ManageStatus();

            if (c % 60 == 0) {
                mnodeman.CheckAndRemove();
                mnodeman.ProcessKarmanodeConnections();
                karmanodePayments.CleanPaymentList();
                CleanTransactionLocksList();
            }

            //if(c % KARMANODES_DUMP_SECONDS == 0) DumpKarmanodes();
