...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "ohmcoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "ohmcoind.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the OHMC and zOHMC money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 52020, 11773));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in OHMC/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in OHMC/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in OHMC/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Ohmcoin specific functionality (Karmanodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageGroup(_("Karmanode options:"));
    strUsage += HelpMessageOpt("-karmanode=<n>", strprintf(_("Enable the client to act as a karmanode (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf(_("Specify karmanode configuration file (default: %s)"), "karmanode.conf"));
    strUsage += HelpMessageOpt("-mnconflock=<n>", strprintf(_("Lock karmanodes from karmanode configuration file (default: %u)"), 1));
    strUsage += HelpMessageOpt("-karmanodeprivkey=<n>", _("Set the karmanode private key"));
    strUsage += HelpMessageOpt("-karmanodeaddr=<n>", strprintf(_("Set external address:port to get to this karmanode (example: %s)"), "128.127.106.235:52020"));
    ...
    strUsage += HelpMessageOpt("-backupzohmc=<n>", strprintf(_("Enable automatic wallet backups triggered after each zOhmc minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zohmcbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zOHMC backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif
//    strUsage += "  -anonymizeohmcoinamount=<n>     " + strprintf(_("Keep N OHMC anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Ohmcoin Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Ohmcoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Ohmcoin Core is shutting down."));
...
#endif
    // Make sure only a single Ohmcoin process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Ohmcoin Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    if (!fDisableWallet) {
        ...
        if (!karmanodeConfig.read(strErr)) {
            string msg = strprintf("Error reading karmanode configuration file: %s\n", strErr.c_str());
            return InitError(msg);
        }
    }  // (!fDisableWallet)
#endif // ENABLE_WALLET
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //Ohmcoin specific: zerocoin and spork DB's
                ...
                // Ohmcoin: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZOHMCMinted();
                        RecalculateZOHMCSpent();
                    }
                    RecalculateOHMCSupply(1);
                }
                ...
                // Ohmcoin: recalculate Accumulator Checkpoints that failed to database properly
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Ohmcoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Ohmcoin Core to complete") << "\n";
        ...
        zwalletMain = new CzOHMCWallet(pwalletMain->strWalletFile);
        ...
        uiInterface.InitMessage(_("Syncing zOHMC wallet..."));

        bool fEnableZOhmcBackups = GetBoolArg("-backupzohmc", true);
        pwalletMain->setZOhmcAutoBackups(fEnableZOhmcBackups);
    ...
    uiInterface.InitMessage(_("Loading karmanode cache..."));

    CKarmanodeDB mndb;
    CKarmanodeDB::ReadResult readResult = mndb.Read(mnodeman);
    ...
    uiInterface.InitMessage(_("Loading karmanode payment cache..."));

    CKarmanodePaymentDB mnpayments;
    CKarmanodePaymentDB::ReadResult readResult3 = mnpayments.Read(karmanodePayments);

    if (readResult3 == CKarmanodePaymentDB::FileError)
        LogPrintf("Missing karmanode payment cache - mnpayments.dat, will try to recreate\n");
    else if (readResult3 != CKarmanodePaymentDB::Ok) {
        LogPrintf("Error reading mnpayments.dat: ");
        if (readResult3 == CKarmanodePaymentDB::IncorrectFormat)
            LogPrintf("magic is ok but data has invalid format, will try to recreate\n");
        else
            LogPrintf("file format is unknown or invalid, please fix it manually\n");
    }

    fMasterNode = GetBoolArg("-karmanode", false);

    if ((fMasterNode || karmanodeConfig.getCount() > -1) && fTxIndex == false) {
        return InitError("Enabling Karmanode support requires turning on transaction indexing."
                         "Please add txindex=1 to your configuration and start with -reindex");
    }

    if (fMasterNode) {
        LogPrintf("IS MASTER NODE\n");
        strMasterNodeAddr = GetArg("-karmanodeaddr", "");
        ...
        if (!strMasterNodeAddr.empty()) {
            ...
            if (!addrTest.IsValid()) {
                return InitError("Invalid -karmanodeaddr address: " + strMasterNodeAddr);
            }
        }
        ...
        if (!strKarmaNodePrivKey.empty()) {
            ...
            activeKarmanode.pubKeyKarmanode = pubkey;

        } else {
            return InitError(_("You must specify a karmanodeprivkey in the configuration. Please see documentation for help."));
        }
    }

    //get the mode of budget voting for this karmanode
    strBudgetMode = GetArg("-budgetvotemode", "auto");

    if (GetBoolArg("-mnconflock", true) && pwalletMain) {
        LOCK(pwalletMain->cs_wallet);
        LogPrintf("Locking Karmanodes:\n");
        uint256 mnTxHash;
...
//    }
//
//    nAnonymizeOhmcoinAmount = GetArg("-anonymizeohmcoinamount", 0);
//    if (nAnonymizeOhmcoinAmount > 999999) nAnonymizeOhmcoinAmount = 999999;
//    if (nAnonymizeOhmcoinAmount < 2) nAnonymizeOhmcoinAmount = 2;

    fEnableSwiftTX = GetBoolArg("-enableswifttx", fEnableSwiftTX);
    nSwiftTXDepth = GetArg("-swifttxdepth", nSwiftTXDepth);
    nSwiftTXDepth = std::min(std::max(nSwiftTXDepth, 0), 60);

    //lite mode disables all Karmanode and Obfuscation related functionality
    fLiteMode = GetBoolArg("-litemode", false);
    if (fMasterNode && fLiteMode) {
        return InitError("You can not start a karmanode in litemode");
    }

    LogPrintf("fLiteMode %d\n", fLiteMode);
    LogPrintf("nSwiftTXDepth %d\n", nSwiftTXDepth);
    LogPrintf("Anonymize Ohmcoin Amount %d\n", nAnonymizeOhmcoinAmount);
    ...
    /* Denominations
       ...
       For example:
       1OHMC+1000 == (.1OHMC+100)*10
       10OHMC+10000 == (1OHMC+1000)*10
    */
