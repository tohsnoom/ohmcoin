...
//step 2.) Top SWIFTTX_SIGNATURES_TOTAL karmanodes, open connect to top 1 karmanode.
//         Send "txvote", CTransaction, Signature, Approve
//step 3.) Top 1 karmanode, waits for SWIFTTX_SIGNATURES_REQUIRED messages. Upon success, sends "txlock'
...
void ProcessMessageSwiftTX(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    } else if (strCommand == NetMsgType::TXLVOTE) // SwiftX Lock Consensus Votes
    {
        ...
        if (ProcessConsensusVote(pfrom, ctx)) {
            //Spam/Dos protection
            /*
                Karmanodes will sometimes propagate votes before the transaction is known to the client.
            ...
            */
            if (!mapTxLockReq.count(ctx.txHash) && !mapTxLockReqRejected.count(ctx.txHash)) {
                if (!mapUnknownVotes.count(ctx.vinKarmanode.prevout.hash)) {
                    mapUnknownVotes[ctx.vinKarmanode.prevout.hash] = GetTime() + (60 * 10);
                }

                if (mapUnknownVotes[ctx.vinKarmanode.prevout.hash] > GetTime() &&
                    mapUnknownVotes[ctx.vinKarmanode.prevout.hash] - GetAverageVoteTime() > 60 * 10) {
                    LogPrintf("ProcessMessageSwiftTX::ix - karmanode is spamming transaction votes: %s %s\n",
                        ctx.vinKarmanode.ToString().c_str(),
...
int64_t CreateNewLock(CTransaction tx)
{
    ...
    /*
        ...
        This prevents attackers from using transaction mallibility to predict which karmanodes
        they'll use.
    */
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    int n = mnodeman.GetKarmanodeRank(activeKarmanode.vin, nBlockHeight, MIN_SWIFTTX_PROTO_VERSION);

    if (n == -1) {
        LogPrint("swiftx", "SwiftX::DoConsensusVote - Unknown Karmanode\n");
        return;
    }

    if (n > SWIFTTX_SIGNATURES_TOTAL) {
        LogPrint("swiftx", "SwiftX::DoConsensusVote - Karmanode not in the top %d (%d)\n", SWIFTTX_SIGNATURES_TOTAL, n);
        return;
    }
    /*
    ...
    */
    ...
    CConsensusVote ctx;
    ctx.vinKarmanode = activeKarmanode.vin;
...
{
    int n = mnodeman.GetKarmanodeRank(ctx.vinKarmanode, ctx.nBlockHeight, MIN_SWIFTTX_PROTO_VERSION);

    CKarmanode* pmn = mnodeman.Find(ctx.vinKarmanode);
    if (pmn != NULL)
        LogPrint("swiftx", "SwiftX::ProcessConsensusVote - Karmanode ADDR %s %d\n", pmn->addr.ToString().c_str(), n);

    if (n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("swiftx", "SwiftX::ProcessConsensusVote - Unknown Karmanode\n");
        mnodeman.AskForMN(pnode, ctx.vinKarmanode);
        return false;
    }

    if (n > SWIFTTX_SIGNATURES_TOTAL) {
        LogPrint("swiftx", "SwiftX::ProcessConsensusVote - Karmanode not in the top %d (%d) - %s\n", SWIFTTX_SIGNATURES_TOTAL, n, ctx.GetHash().ToString().c_str());
        return false;
    }

    if (!ctx.SignatureValid()) {
        LogPrintf("SwiftX::ProcessConsensusVote - Signature invalid\n");
        // don't ban, it could just be a non-synced karmanode
        mnodeman.AskForMN(pnode, ctx.vinKarmanode);
        return false;
    }
...
{
    return vinKarmanode.prevout.hash + vinKarmanode.prevout.n + txHash;
}
...
bool CConsensusVote::SignatureValid()
{
    ...
    if (pmn == NULL) {
        LogPrintf("SwiftX::CConsensusVote::SignatureValid() - Unknown Karmanode\n");
        return false;
    }

    if (!obfuScationSigner.VerifyMessage(pmn->pubKeyKarmanode, vchMasterNodeSignature, strMessage, errorMessage)) {
...
bool CConsensusVote::Sign()
{
    ...
    //LogPrintf("signing privkey %s \n", strKarmaNodePrivKey.c_str());

    if (!obfuScationSigner.SetKey(strKarmaNodePrivKey, errorMessage, key2, pubkey2)) {
        LogPrintf("CConsensusVote::Sign() - ERROR: Invalid karmanodeprivkey: '%s'\n", errorMessage.c_str());
        return false;
    }
...
bool CTransactionLock::SignaturesValid()
{
    BOOST_FOREACH (CConsensusVote vote, vecConsensusVotes) {
        int n = mnodeman.GetKarmanodeRank(vote.vinKarmanode, vote.nBlockHeight, MIN_SWIFTTX_PROTO_VERSION);

        if (n == -1) {
            LogPrintf("CTransactionLock::SignaturesValid() - Unknown Karmanode\n");
            return false;
        }

        if (n > SWIFTTX_SIGNATURES_TOTAL) {
            LogPrintf("CTransactionLock::SignaturesValid() - Karmanode not in the top %s\n", SWIFTTX_SIGNATURES_TOTAL);
            return false;
        }
