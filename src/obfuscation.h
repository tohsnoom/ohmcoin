...
#define KARMANODE_ACCEPTED 1
#define KARMANODE_REJECTED 0
#define KARMANODE_RESET -1
...
extern std::string strKarmaNodePrivKey;
extern map<uint256, CObfuscationBroadcastTx> mapObfuscationBroadcastTxes;
extern CActiveKarmanode activeKarmanode;
...
public:
     ...
     *     1) we have an active Karmanode,
     *     2) we have a valid Karmanode private key,
    ...
    /// Check if we have a valid Karmanode address
    bool CheckSignature();
};
...
{
public:
    /// Is the inputs associated with this public key? (and there is 10000 OHMC - checking if valid karmanode)
...
private:
    ...
    std::vector<CObfuScationEntry> entries; // Karmanode/clients entries
    ...
    bool sessionFoundKarmanode; //If we've found a compatible Karmanode
...
public:
    ...
    CKarmanode* pSubmittedToKarmanode;
    ...
    void UpdateState(unsigned int newState)
    {
        if (fMasterNode && (newState == POOL_STATUS_ERROR || newState == POOL_STATUS_SUCCESS)) {
            // LogPrint("obfuscation", "CObfuscationPool::UpdateState() - Can't set state to ERROR or SUCCESS as a Karmanode. \n");
            return;
        }
    ...
    /// As a client, send a transaction to a Karmanode to start the denomination process
    void SendObfuscationDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, CAmount amount);
    /// Get Karmanode updates about the progress of Obfuscation
    ...
    /// Get the denominations for a specific amount of ohmcoin.
